...
import com.example.frameimpl.ApplicationImpl;
...
public class MemoryUtils {
	...
	private static final int MAXMEMORY = 300 * 1024 * 1024;// 程序运行的最大内存
	...
	/**
	 * 获取系统内存信息 android 4.1以上系统可以获得最大内存
	 * 
	 * @return
	 */

	public static StringBuilder getMemoryInfo() {
		final ActivityManager activityManager = (ActivityManager) ApplicationImpl.getInstance().getSystemService(Context.ACTIVITY_SERVICE);
		ActivityManager.MemoryInfo info = new ActivityManager.MemoryInfo();
		activityManager.getMemoryInfo(info);
		StringBuilder builder = new StringBuilder();
		builder.append("=====内容数据仅供参考，未必准确=====");

		if (android.os.Build.VERSION.SDK_INT >= 16) { 
			builder.append("系统给最大内存：").append(info.totalMem >> 10).append("k").append("\n");
		}

		builder.append("系统剩余内存：").append(info.availMem >> 10).append("k").append("\r\n");
		builder.append("系统跟是否处于低内存运行：").append(info.lowMemory).append("\r\n");
		builder.append("当系统剩余内存低于").append(info.threshold).append("时就视为低内存运行");
		return builder;
	}
